...
\begin{abstract}
	...
	Modern robotics control systems often seek to eliminate users from the control loop through the design of smarter and more autonomous robots able to reason and path-plan with little input from the controllers, so as to execute tasks without the need for additional input. In such systems, however, users are often functionally isolated from the essential decision making pathway, which is unfavorable for applications where the direct involvement of human operators is desired in the control of a robot.
	Traditional input mechanisms are often developed with a desire to simplify the geometric complexities of the robots they control by providing low-bandwidth control mechanisms to users. However, the desire to simplify such complexities can often limit the extent of control and thus hinder the capacity of the robot operator in delivering the intended outcome.  
	To address the challenge of developing a more robust yet user-friendly input device, we employ biomimetic design elements, through which the control mechanism can traverse the barrier between a passive input sensing apparatus to a complex, dynamic system for robust-human interfacing. We introduce the design of a novel, intuitive and organic input device for robotic control and human-robot interfacing applications. We have developed a novel hand-held input device, the TetroCon, the movements of which can be mapped onto robotic structures to allow for enhanced control and accessibility to the functionality of robots while maintaining a practical user-friendliness and simplicity.
